...
|---|---|---|---|
| 게시글 목록 조회 | `GET` | `/posts` | 모든 게시글을 조회합니다. |
| 게시글 상세 조회 | `GET` | `/posts/{post_id}` | ID에 해당하는 게시글 하나를 조회합니다. |
| 게시글 작성 | `POST` | `/posts` | 새로운 게시글을 작성합니다. |
| 게시글 수정 | `PUT` | `/posts/{post_id}` | 기존 게시글 전체를 수정합니다. |
| 게시글 삭제 | `DELETE` | `/posts/{post_id}` | 기존 게시글을 삭제합니다. |
...
- 리소스 이름을 `memo`에서 `post`로 변경했습니다.
- 게시글에 `author` 필드를 추가했습니다.
- REST API 경로를 명사형 `/posts`로 통일했습니다.
- 수정할 게시글 ID를 요청 본문이 아니라 `/posts/{post_id}` 경로에서 받습니다.
